...
#if MODIFY(ENGINE)
            {
                m_redirectedWindow->setIsAppMode(webkit_web_view_app_mode(NULL) == WEBKIT_LAUNCHER_MODE_APP);    
...
void AcceleratedCompositingContext::compositeLayersToContext(CompositePurpose purpose)
{
    ...
    if (!context)
        return;
#if USE(ACCELERATED_COMPOSITING) && ENABLE(ODROID)
    reinterpret_cast<GLContextEGL*>(context)->clientWaitSyncKHR();
#endif
...
#if USE(ACCELERATED_COMPOSITING) && ENABLE(ODROID)
    reinterpret_cast<GLContextEGL*>(context)->destroySyncKHR();
#endif
}
...
#if USE(ACCELERATED_COMPOSITING) && ENABLE(ODROID)
    if(context)
        reinterpret_cast<GLContextEGL*>(context)->createSyncKHR();
#endif
