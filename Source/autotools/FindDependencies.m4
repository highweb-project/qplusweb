...
case "$host" in
    ...
    *-*-mingw*)
        ...
        ;;
    *-*-gnueabihf*)
	UNICODE_CFLAGS=`$PKG_CONFIG --cflags icu-uc icu-i18n icu-le`
	UNICODE_LIBS=`$PKG_CONFIG --libs icu-uc icu-i18n icu-le`
	;;
    *)
...
ATK_BRIDGE_LIBS=`$PKG_CONFIG --libs atk-bridge-2.0`
...
if test "$found_opengl" = "yes"; then
    ...
    if test "$enable_gles2" = "yes"; then
        ...
        OPENGL_LIBS="-lGLESv2"
	# [2013.08.15][infraware] ARM Cross Compile Odroid-XU3 Mali-T628 Graphics
	if test "$host" = "arm-unknown-linux-gnueabihf"; then	    
		OPENGL_LIBS+=" -lmali -L/usr/lib/arm-linux-gnueabihf/mali-egl"	    
	fi
