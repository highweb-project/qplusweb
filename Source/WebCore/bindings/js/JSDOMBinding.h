...
#if MODIFY(ENGINE)
template<>
struct JSValueTraits<long long> {
    static inline JSC::JSValue arrayJSValue(JSC::ExecState*, JSDOMGlobalObject*, const long long& value)
    {
        return JSC::jsNumber(value);
    }
};
...
#if MODIFY(ENGINE)
template<>
struct NativeValueTraits<long long> {
    static inline bool nativeValue(JSC::ExecState* exec, JSC::JSValue jsValue, long long& indexedValue)
    {
        indexedValue = static_cast<long long>(jsValue.toNumber(exec));
        return !exec->hadException();
    }
};
#endif
...
{
#if MODIFY(ENGINE)
    //[2014.09.18][infraware][hyunseok] merge webkit-2.5.1
    JSC::JSObject* object = value.getObject();
    if (!object)
        return Vector<T>();
#endif
...
#if MODIFY(ENGINE)
template <class T>
Vector<T> toNativeArrayForWebCLArgument(JSC::ExecState* exec, JSC::JSValue value)
{
    //[2014.09.18][infraware][hyunseok] merge webkit-2.5.1
    JSC::JSObject* object = value.getObject();
    if (!object){
