...
#if MODIFY(ENGINE)
//[2014.03.05][infraware][jungong16] : fix to issue. http://www.w3.org/html/wg/drafts/html/CR/editing.html#the-datatransfer-interface
static String convertURIListToURL(const String& uriList, bool convertToURL = FALSE)
{
...
#if MODIFY(ENGINE)
PassOwnPtr<Pasteboard> Pasteboard::createForDragAndDrop(PassRefPtr<Clipboard> clipboard, ScriptExecutionContext* context)
{
    return create(DataObjectGtk::create(clipboard, context));
}
#else
...
#if MODIFY(ENGINE)
    if (type == "Files" || type == "text/uri-list" || type.startsWith("text/uri-list;") || type == "url")
#else
...
#if MODIFY(ENGINE)
        //[2014.03.03][infraware][jungong16] : fix to issue. http://www.w3.org/TR/clipboard-apis/#mandatory-data-types-1
#else
...
#if MODIFY(ENGINE)
        //[2014.03.05][infraware][jungong16] : fix to issue. http://www.w3.org/html/wg/drafts/html/CR/editing.html#the-datatransfer-interface
        bool convertToURL = TRUE;
        return convertURIListToURL(m_dataObject->uriList(), convertToURL);
#else
