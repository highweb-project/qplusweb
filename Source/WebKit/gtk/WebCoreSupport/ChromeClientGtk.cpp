...
#if MODIFY(ENGINE)
void ChromeClient::elementDidFocus(Node* node)
{   
#if ENABLE(ODROID)
    bool openVirtualKeyboard = false;
    if(node && node->hasTagName(HTMLNames::inputTag))
    {        
         HTMLInputElement* input = static_cast<HTMLInputElement*>(node);
        if(input)
            openVirtualKeyboard = input->isText();
    }    
    else
        openVirtualKeyboard = node->isContentEditable();

    if(openVirtualKeyboard)
    {
        char* pKeyboard = "/usr/bin/onboard";
        FILE* fp = fopen(pKeyboard, "rb");
...
#if MODIFY(ENGINE)
#if ENABLE(INPUT_TYPE_COLOR)
PassOwnPtr<ColorChooser> ChromeClient::createColorChooser(WebCore::ColorChooserClient* client, const Color& color)
{
    if(!client)
        return nullptr;

    m_chooserClient = client;
	return adoptPtr(new ColorChooserGtk(this, color));
}

void ChromeClient::removeColorChooser()
{
    if(m_chooserClient)
	   m_chooserClient->didEndChooser();
}

void ChromeClient::updateColorChooser(const Color& color)
{	
    if(m_chooserClient)
        m_chooserClient->didChooseColor(color);
}
...
PassRefPtr<WebCore::DateTimeChooser> ChromeClient::openDateTimeChooser(DateTimeChooserClient* client, const DateTimeChooserParameters& parameter)
{
    if(!client)
        return nullptr;
    ...
    isDateType = (type)==("date") || (type)==("datetime") || (type)==("month") || (type)==("week") || (type)==("datetime-local");
    isTimeType = (type)==("time") || (type)==("datetime") || (type)==("datetime-local");
   
    if(isDateType && isTimeType){
        return adoptRef(new DateTimeChooserGtk(this, type));
    }
    else if(isTimeType){
        return adoptRef(new TimeChooserGtk(this, type));
    }
    else if(isDateType){
        return adoptRef(new DateChooserGtk(this, type));
    }
    
    return nullptr;
}

void ChromeClient::removeDateTimeChooser()
{
...
void ChromeClient::updateDateTimeChooser(const String& value)
{
    if(m_datetimechooserClient)
        m_datetimechooserClient->didChooseValue(value);
}

bool ChromeClient::isAppMode() 
{ 
    return webkit_web_view_app_mode(NULL) == WEBKIT_LAUNCHER_MODE_APP;
}

#endif
#endif //MODIFY(ENGINE)

void ChromeClient::enableWebGL()
{
    g_signal_emit_by_name(m_webView, "enabled-webgl", 0,0);
    // webkit_web_view_enable_webgl(m_webView);
}

void ChromeClient::enableWebCL()
{
    g_signal_emit_by_name(m_webView, "enabled-webcl", 0,0);
    // webkit_web_view_enable_webcl(m_webView);
}
