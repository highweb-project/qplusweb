...
enum {
    ...
    ENABLED_WEBGL,
    ENABLED_WEBCL,
    USER_MEDIA_POLICY_DECISION_REQUESTED,
    LAST_SIGNAL
};
...
#if MODIFY(ENGINE)
static gboolean hide_scrollbar = FALSE;
static WebKitLauncherMode g_mode = WEBKIT_LAUNCHER_MODE_BROWSER;
#endif
...
{
#if MODIFY(ENGINE)
    if(webkit_web_view_app_mode(WEBKIT_WEB_VIEW(widget)) == WEBKIT_LAUNCHER_MODE_BROWSER)
        return FALSE;
#endif
...
#if MODIFY(ENGINE)
        if(webkit_web_view_app_mode(webView) == WEBKIT_LAUNCHER_MODE_BROWSER)
#endif
...
#if ENABLE(CONTEXT_MENUS)
#if MODIFY(ENGINE)
    if (event->button == 3 && webkit_web_view_app_mode(webView) == WEBKIT_LAUNCHER_MODE_BROWSER)
#else
    if (event->button == 3)
#endif
...
static void resizeWebViewFromAllocation(WebKitWebView* webView, GtkAllocation* allocation, bool sizeChanged)
{
    ...
    IntSize oldSize;
#if MODIFY(ENGINE)
    WebKitLauncherMode launcherMode = webkit_web_view_app_mode(webView);
#endif
    FrameView* frameView = page->mainFrame().view();
    if (sizeChanged && frameView) {
        oldSize = frameView->size();
#if MODIFY(ENGINE)
        if(launcherMode == WEBKIT_LAUNCHER_MODE_BROWSER )
#endif
...
static void webkit_web_view_class_init(WebKitWebViewClass* webViewClass)
{
...
#if MODIFY(ENGINE)
    webkit_web_view_signals[ENABLED_WEBGL] = g_signal_new("enabled-webgl",
        G_TYPE_FROM_CLASS(webViewClass),
        (GSignalFlags)(G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION),
        G_STRUCT_OFFSET(WebKitWebViewClass, enable_webgl),
        NULL, NULL,
        g_cclosure_marshal_VOID__VOID,
        G_TYPE_NONE, 0);

    webkit_web_view_signals[ENABLED_WEBCL] = g_signal_new("enabled-webcl",
        G_TYPE_FROM_CLASS(webViewClass),
        (GSignalFlags)(G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION),
        G_STRUCT_OFFSET(WebKitWebViewClass, enable_webcl),
        NULL, NULL,
        g_cclosure_marshal_VOID__VOID,
        G_TYPE_NONE, 0);

    webkit_web_view_signals[USER_MEDIA_POLICY_DECISION_REQUESTED] = g_signal_new("user-media-policy-decision-derequested",
        G_TYPE_FROM_CLASS(webViewClass),
        (GSignalFlags)(G_SIGNAL_RUN_LAST),
        0,
        NULL, NULL,
    ...
    // webViewClass->enable_webgl = webkit_web_view_enable_webgl;
    // webViewClass->enable_webcl = webkit_web_view_enable_webcl;
#if MODIFY(ENGINE)
    webViewClass->enable_webgl = NULL;
    webViewClass->enable_webcl = NULL;
#endif
...
#if ENABLE(WEBCL)
    coreSettings.setWebCLEnabled(settingsPrivate->enableWebcl);
#endif
...
#if ENABLE(WEBCL)
    else if (name == g_intern_string("enable-webcl")){
        settings.setWebCLEnabled(g_value_get_boolean(&value));
    }
#endif
...
#if MODIFY(ENGINE)
void webkit_web_view_hide_scrollbar(WebKitWebView* webView)
{
    hide_scrollbar = TRUE;
}

void webkit_web_view_set_app_mode(WebKitWebView* web_veiw, WebKitLauncherMode mode)
{
    g_mode = mode;
}

WebKitLauncherMode webkit_web_view_app_mode(WebKitWebView *web_view)
{
    return g_mode;

}

// void webkit_web_view_enable_webgl(WebKitWebView* webView)
// {
// }

// void webkit_web_view_enable_webcl(WebKitWebView* webView)
// {
// }
